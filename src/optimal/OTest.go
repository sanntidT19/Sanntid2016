...
func main() {
	...
	var C [ELEV]Cabin
	//var C[N_ELEV]Slave <- InMasterChans.OptimizationInit
	...
	dire := []bool{true, true, true}
	posit := []int{3, 2, 2}

	// Input from inside cabins
	InBut := []bool{false, false, false, false,
	...
	for i := 0; i < ELEV; i++ {
		...
		Y = (Allready(H, C[i].Buttons[0:FLOORS], C[i].Direction, C[i].Position))
